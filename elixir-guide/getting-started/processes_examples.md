...
```elixir
iex(1)> # The function spawn/1 does the basic mechanism of spawning new processes
iex(2)> spawn fn -> 4 + 12 end
#PID<0.108.0>
iex(3)> self() # Will generate the PID of the current process
#PID<0.105.0>
iex(4)> # We can check if a process is alive through  Process.alive()
...
true
iex(8)> # Send and Recieve allows us to send messages between processes
...
:delete_me
:ok
iex(16)> # The function spawn_link/1 allows us to link processes, so if something fails in one, the failure propagates to the others
iex(17)> self()
#PID<0.105.0>
...
** (RuntimeError) horrible horrible error
    (stdlib 3.12.1) erl_eval.erl:678: :erl_eval.do_apply/6
iex(1)> # Tasks are built on top of spawn functions and give better error reports
nil
